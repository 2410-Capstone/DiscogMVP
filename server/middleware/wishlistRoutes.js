...
const {createWishlist, getWishlistById, getWishlistByShareId, addProductToWishlist, removeProductFromWishlist, getProductsInWishlist, getUserWishlists, updateWishlist, deleteWishlist, isProductInWishlist} = require('../db/wishlist')
const authenticate = require('../middleware/authMiddleware');


router.get('/user/:userId', authenticate, async (req, res) => {
  try {
    console.log("Fetching wishlists for:", req.params.userId);
    ...
    console.error("Error in /user/:userId:", {
      ...
      stack: error.stack,
      query: error.query // For database errors
    });
    res.status(500).json({ 
      error: "Server error",
